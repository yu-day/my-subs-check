...
func Check() ([]Result, error) {
	...
	// ===== 新增：低配设备节点数量限制 =====
	if config.GlobalConfig.MaxProxiesPerCheck > 0 && len(proxies) > config.GlobalConfig.MaxProxiesPerCheck {
		slog.Warn(fmt.Sprintf("节点数量 %d 超过限制 %d，本轮仅检测前 %d 个节点",
			len(proxies), config.GlobalConfig.MaxProxiesPerCheck, config.GlobalConfig.MaxProxiesPerCheck))
		proxies = proxies[:config.GlobalConfig.MaxProxiesPerCheck]
	}
	// ===== 新增结束 =====
...
func (pc *ProxyChecker) run(proxies []map[string]any) ([]Result, error) {
	...
	// 替换原有的参数日志，更清晰地显示哪些功能被启用
	mediaEnabled := config.GlobalConfig.MediaCheck
	speedEnabled := config.GlobalConfig.SpeedTestUrl != ""
	slog.Info("当前参数",
		"timeout", config.GlobalConfig.Timeout,
		"concurrent", config.GlobalConfig.Concurrent,
		"media-check", mediaEnabled,
		"speed-test", speedEnabled,
		"min-speed", config.GlobalConfig.MinSpeed,
		"download-timeout", config.GlobalConfig.DownloadTimeout,
	)
	if !mediaEnabled {
		slog.Info("媒体检测已关闭，节省带宽")
	}
	if !speedEnabled {
		slog.Info("测速已关闭，节省带宽")
	}
	...
	speedTestURL := config.GlobalConfig.SpeedTestUrl //测速开关
	hasSpeedTest := speedTestURL != ""                //把 SpeedTestUrl 置空就等于关闭测速
	total := len(proxies)
...
func (pc *ProxyChecker) checkMedia(a aliveResult) *Result {
	...
	//媒体开关：
	if config.GlobalConfig.MediaCheck {   //MediaCheck: false 就关闭媒体检测
